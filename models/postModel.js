...
module.exports = {
    ...
    createPost: async (level, data) => {
        ...
        const [createResult] = await db.query(query, [level, title, content, category]);
        
        return createResult.insertId;
    },
    updatePost: async (id, data) => {
        const {title, content, category} = data;
        const query = 'UPDATE Post SET title=?, conetent=?, category=? WHERE post_id=?;';
        const [updateResult] = await db.query(query, [title, content, category, id]);
